...
void MattsLedStrip::SetBrightness(int val){
        maxB = val;
        rememberB = maxB;
        chsv.value = maxB;
}
...
void MattsLedStrip::RandomPattern(){
...
void MattsLedStrip::RandomProperties(){
        if(program_number == 1) mod = random8(1,100);
        if(program_number == 2) mod = random8(1,9);
}

/*If specific programs need properties adjusted for proper functioning, we do that here.*/
...
void MattsLedStrip::AudioLevelCheck(){
        bumpNow = millis();
...
void MattsLedStrip::AudioFadeOut(){
        if(maxB >= fadeFactor)
        {
...
void MattsLedStrip::SetAudioReactive(int val){
        if(val > 0) {
...
void MattsLedStrip::TurnOff(){
        ClearStrip();
        isOn = false;
}

void MattsLedStrip::TurnOn(){
        isOn = true;
}

void MattsLedStrip::TogglePower(){
        if(isOn) TurnOff();
        else TurnOn();
}
